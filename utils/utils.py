...
async def approve_asset(obj, contract, spender, value=None, legacy=False):
    ...
    contract = await Contract(obj.client).get_contract(
        ...
        abi=approve_abi)
    if legacy:
        gas_params = {"gasPrice": await obj.client.w3.eth.gas_price}
    else:
...
async def approve_if_insufficient_allowance(obj, contract_address, spender, value=2**256-1, legacy=False):
    ...
    if current_allowance < value and not current_allowance > 2**236:
        ...
        return await approve_asset(obj, spender, contract_address, legacy=legacy)
